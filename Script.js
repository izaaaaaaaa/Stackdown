...
const T_SPIN_LIMIT = 7; 

let easterEggTriggeredThisPiece = false;

let forceNextPieces = 0; // Contador para o "Modo T-Rex"
 ...
 * E ATIVA O TREMOR DE TELA (Screen Shake).
...
    messageElement.classList.remove('hidden'); // Mostra a mensagem
    ...
    // Aplica o "shake" no canvas

    if (canvas) {

        canvas.classList.add('shake-it');

        

        // Remove a classe após a animação (400ms)

        setTimeout(() => {

            canvas.classList.remove('shake-it');
...
function spawnNewPiece() {
    ...
    let randomName; // 1. Declare a variável aqui

    

    // 2. Adicione este bloco if/else

    if (forceNextPieces > 0) {

        randomName = 'T4'; // Força a peça T

        forceNextPieces--;   // Decrementa o contador

    } else {

        // 3. Esta é a sua linha original, agora dentro do 'else'

        randomName = PIECE_NAMES[Math.floor(Math.random() * PIECE_NAMES.length)];
...
            if (currentPiece.name === 'T4' && !easterEggTriggeredThisPiece) {

                tSpinCounter++;

                

                if (tSpinCounter >= T_SPIN_LIMIT) {
                    ...
                    // 1. Mostra a mensagem E ATIVA O SHAKE (via showEasterEgg)

                    showEasterEgg("MODO T-REX! 🦖 +500");

                    

                    // 2. Toca o som secreto

                    const secretSound = document.getElementById('easterEggSound');

                    if (secretSound) {

                        secretSound.volume = 0.5; // Ajuste o volume se precisar
                    ...
                    // 4. Ativa o "Modo T-Rex"

                    forceNextPieces = 3; // Próximas 3 peças serão 'T'



                    easterEggTriggeredThisPiece = true; // Impede que ative de novo
